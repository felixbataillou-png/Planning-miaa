...
async function loadBenevoles () {
  ...
  const { data, error } = await db
    .from('volunteers')
    .select('id, commentaire, nom, prenom, email, tel, permis, profession, adresse, codepostal, ville, urgence_contact')
  ...
  searchTimer = setTimeout(() => {
    ...
    if (!q) {
      ...
      filteredList = allBenevoles.filter(b =>
        [b.nom, b.prenom, b.email, b.tel, b.profession, b.ville, b.commentaire, b.urgence_contact]
...
function renderTable () {
  ...
  tbody.innerHTML = page.map(b => `
    <tr>
      <td title="${esc(b.commentaire)}">${esc(truncate(b.commentaire, 30))}</td>
...
function openEditModal (id) {
  ...
  document.getElementById('bvl-commentaire').value  = b.commentaire    || ''
...
function resetBvlForm () {
  ;['bvl-commentaire','bvl-nom','bvl-prenom','bvl-email','bvl-tel',
...
async function saveBenévole () {
  ...
  const payload = {
    commentaire:     document.getElementById('bvl-commentaire').value.trim(),
    nom,
    prenom,
